...
const main = async () => {
  console.log(
    "A quick script that will setup a clean localhost instance of Frequency for DSNP ",
  );
  ...
  await new Promise((resolve, reject) => {
    ...
    api.tx.msa
      .createProvider("alice")
      .signAndSend(keys, {}, ({ status, events, dispatchError }) => {
        if (dispatchError) {
          console.error("ERROR: ", dispatchError.toHuman());
          reject();
        } else if (status.isInBlock || status.isFinalized) {
          const evt = eventWithSectionAndMethod(
            events,
            "msa",
            "ProviderCreated",
          );
